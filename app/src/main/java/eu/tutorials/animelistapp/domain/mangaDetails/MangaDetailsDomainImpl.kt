...
@Singleton
class MangaDetailsDomainImpl @Inject constructor(private val mangaRepository: MangaRepository):MangaDetailsDomain {

    override fun getMangaDetails(
        id: Int,
    ...
    override fun getMangaCharacters(mangaId: Int): Flow<Resource<List<MangaCharacter>>> {
        return flow {
            emit(Resource.Loading())
            try {
                val mangaCharacters =
    ...
    override fun getMangaRecommendations(mangaId: Int): Flow<Resource<List<Recommendation>>> {
        return flow {
            emit(Resource.Loading())
            try {
