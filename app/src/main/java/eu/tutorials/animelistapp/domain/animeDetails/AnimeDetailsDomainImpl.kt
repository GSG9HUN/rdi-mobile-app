...
@Singleton
class AnimeDetailsDomainImpl @Inject constructor(private val animeRepository: AnimeRepository):
    AnimeDetailsDomain {
    ...
    override fun getAnimeCharacters(animeId: Int): Flow<Resource<List<AnimeCharacter>>> {
        return flow {
            emit(Resource.Loading())
            try {
                val animeCharacters =
